 ...
 *
 * UPSTREAM: Vendored from posthog.com.
 * Source: https://github.com/PostHog/posthog.com/blob/4d496a0762aa88bc0fc052c5d69a9f00ff0c21b9/src/hooks/productData/relationships.ts
 * PR: https://github.com/PostHog/posthog.com/pull/17019
 *
 * Marketing owns this file upstream. When they edit edges or add products, re-pull
 * the file verbatim — do not edit it in place here. If you must extend it for
 * in-app needs, do so in a sibling file (e.g. handleAdapter.ts) so the diff with
...
export function normalizeHandle(rawRef: string): ProductHandle | null {
    ...
    if (MAIN_PRODUCT_HANDLES.includes(rawRef)) {
        return rawRef
    }
    if (rawRef in SLUG_ALIASES) {
        return SLUG_ALIASES[rawRef]
    }
    return null
}
