...
class TestAIEventsUsageReport(ClickhouseDestroyTablesMixin, TestCase, ClickhouseTestMixin):
    ...
    def test_signals_credits_counts_actionable_reports_with_prs(self) -> None:
        """Signals is billed on outcomes: actionable reports whose implementation shipped a PR in-period.

        Full logic coverage lives in products/signals/backend/test/test_billing.py; this guards
        the usage-report wiring end-to-end against Postgres.
        """
        ...
        # P0 actionable report with a PR in-period → $24 = 2400 credits
        self.assertEqual(result, [(self.org_1_team_1.id, 2400)])
