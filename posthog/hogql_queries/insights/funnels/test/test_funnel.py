...
class TestFOSSFunnelUDF(ClickhouseTestMixin, APIBaseTest):
    ...
    def _signup_event(self, **kwargs):
        _create_event(team=self.team, event="user signed up", **kwargs)

    def _add_to_cart_event(self, **kwargs):
        _create_event(team=self.team, event="added to cart", **kwargs)

    def _checkout_event(self, **kwargs):
        _create_event(team=self.team, event="checked out", **kwargs)
    ...
    def test_funnel_with_any_event(self):
        ...
        _create_person(distinct_ids=["completed_movie"], team_id=self.team.pk)
    ...
    def test_funnel_prop_filters(self):
        ...
        # events
        _create_person(distinct_ids=["with_property"], team_id=self.team.pk)
        ...
        _create_person(distinct_ids=["without_property"], team_id=self.team.pk)
        self._signup_event(distinct_id="without_property")
        self._pay_event(distinct_id="without_property")

        # will add to first step
        _create_person(distinct_ids=["half_property"], team_id=self.team.pk)
