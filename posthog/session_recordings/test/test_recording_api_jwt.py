...
class TestSigningKeySelection:
    """Which key signs (mint) vs which keys verify (recording-api)."""
...
class TestRotationVerification:
    """recording-api verifies against the full key set so tokens stay valid across a rotation."""
...
class TestCrossLanguageContract:
    """Pin the literals the Node verifier hardcodes too (nodejs/src/cdp/utils/jwt-utils.ts
    PosthogJwtAudience.RECORDING_API + the team_id/op claims auth.ts reads), so a drift on either
    side breaks a test. The matching Node test lives in auth.test.ts."""
