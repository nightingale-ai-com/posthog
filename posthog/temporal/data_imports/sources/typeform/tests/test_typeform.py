...
class TestTypeformTransport:
    ...
    @parameterized.expand(
        [
            # Completed-only: no response_type param, cursor stays on the chosen submitted_at.
            ("completed_only", {}, None, "submitted_at"),
            # Partials: response_type param is sent and the cursor falls back to landed_at
            # (submitted_at is null for partial/started responses).
            (
                "with_partials",
                {"response_types": "completed,partial,started"},
    ...
    def test_typeform_source_responses_response_type_wiring(
        ...
        assert kwargs["incremental_field"] == expected_incremental_field
