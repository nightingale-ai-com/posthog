...
impl RawNativeFrame {
        ...
        C: SymbolCatalog<OrChunkId<NativeRef>, ParsedNativeSymbols>,
    {
        ...
        match self
            .resolve_impl(team_id, catalog, instruction_addr, debug_images)
            .await
        {
            Ok(frames) => Ok(frames),
...
mod test {
    ...
    async fn test_native_symbolication_elf_non_pie(db: sqlx::PgPool) {
        ...
        let resolved = frame
            .resolve(1, &catalog, &debug_images)
            .await
            .unwrap()
            .pop()
            .unwrap();
    ...
    async fn test_native_inlined_frame_expansion_with_source(db: sqlx::PgPool) {
        ...
        let frames = frame.resolve(1, &catalog, &debug_images).await.unwrap();
    ...
    async fn test_native_rust_symbol_demangling(db: sqlx::PgPool) {
        ...
        const ELF_ZST: &[u8] = include_bytes!("../../tests/static/native/test_rust_binary.zst");
